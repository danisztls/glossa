...
# --------------------------------------------------------------------------
# The export, for the reading view (research note, proposal §5)
# --------------------------------------------------------------------------

#: Where the site reads this table. The mirror of
#: `site/scripts/export-versification.mjs`, which sends the site's tables the
#: other way for the same reason: one owner, one generated copy, and a check
#: that fails when the copy falls behind. Ownership here is Python's, because
#: `KINDS` is a review a person wrote beside the tool that verifies it.
EXPORT_PATH = (
    Path(__file__).resolve().parents[3] / "site" / "src" / "lib" / "divergence.json"
)


def export_table() -> str:
    """The JSON text, byte for byte -- `main` compares against this.

    WHAT CROSSES AND WHAT DOES NOT. The `kind` crosses, because it is the one
    field that says what a reader is owed, and the site turns it into a
    sentence in the reader's own language. The `why` does not: it is English
    prose written for whoever reads this table next, and shipping it would put
    one language's editorial note in front of forty languages' readers.

    THE ROW IS ABOUT THE ADDRESS, NOT ABOUT AN EDITION, which is why no row
    names one. `docs/decisions.md` #2 made reader URLs edition-free --
    `/scriptura/2thess/2?v=16` names a verse and not an edition's verse -- so
    a chapter the editions divide differently is ambiguous for every reader of
    it, including one reading an edition this table was not measured over."""
    chapters: dict[str, dict[str, dict[str, str]]] = {}
    for (osis, chapter), (kind, _why) in {**KINDS, **SILENT}.items():
        row: dict[str, str] = {"kind": kind}
        if (osis, chapter) in MAPPINGS:
            row["mapping"] = MAPPINGS[(osis, chapter)]
        chapters.setdefault(osis, {})[str(chapter)] = row
    doc = {
        "$comment": [
            "Chapters the Bible editions in this corpus divide differently, by kind.",
            "GENERATED by pipeline/scrapers/bible/divergence.py --export; do not edit.",
            "That script fails when this file falls behind the table it exports, and",
            "docs/research/bible-edition-divergence.md is what explains the kinds.",
            "",
            "A row is about the ADDRESS and names no edition: reader URLs are",
            "edition-free (docs/decisions.md #2), so a chapter divided differently by",
            "the editions here is ambiguous for whoever follows a citation into it.",
        ],
        "measured_over": [EDITIONS["en"], EDITIONS["pt"]],
        "chapters": {
            osis: dict(sorted(rows.items(), key=lambda kv: int(kv[0])))
            for osis, rows in sorted(chapters.items())
        },
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--export",
        action="store_true",
        help="Rewrite site/src/lib/divergence.json from the table above.",
    )
    args = parser.parse_args()
    if args.export:
        EXPORT_PATH.write_text(export_table(), encoding="utf-8")
        print(f"[divergence] wrote {EXPORT_PATH}")
        return 0
    require_corpus()
    ...
    exported = EXPORT_PATH.read_text(encoding="utf-8") if EXPORT_PATH.exists() else ""
    if exported != export_table():
        print(
            f"\nEXPORT: {EXPORT_PATH.name} no longer matches the table above\n"
            "  -- re-run `./divergence.py --export`; the reading view serves it"
        )
    ...
    if unclassified or stale or esther or exported != export_table():
        return 1
