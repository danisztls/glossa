...
- **The Chinese edition is the one read from a PDF, and it declares five paragraphs it cannot give.** §1725 the book does not print: its 撮要 (IN BRIEF) heading stands where it belongs and the sequence runs 1724 → 撮要 → 1726. §§2267, 2268, 2396 and 2397 are in the file that carries the 2018 revision of the death-penalty paragraph, and that revision was pasted over the 1997 text without removing it — both texts are in the content stream on the same baselines, and runs of the revision have no text layer at all, so no reader recovers them. What an extractor returns reads as prose and is not, so `ccc_pdf` finds them by geometry and refuses them. It is also the only edition whose `quote` blocks come from type size: its own §21 says the smaller type marks citations from the Fathers, the liturgy, the Magisterium and the saints, which is information the eight HTML editions do not carry. It prints no footnote apparatus and no abbreviations table.
...
- **`items` and `lines` are exclusive, and the difference is what the SOURCE marked.** `items` is a list, by any of four marks: the edition numbered them, set each in its own table cell, set a dash before each (Spanish's beatitudes), or set each on its own line with no sentence anywhere among them (Slovenian's `vera / upanje / ljubezen`). `lines` is a passage. `numbered` says whether the numerals the parse stripped should be redrawn.
- **A line break inside a passage is the printer's, and is not reproduced.** Six editions wrap the Beatitudes at their own column width — Portuguese breaks each one after its comma, Italian and Romanian mid-clause — so `shape_lines` joins a line to the next unless it ended a sentence. What a consumer gets is whole sentences; English, which already sets one beatitude per line, comes through the same rule untouched. Where a beatitude ENDS is still the source's to say and not ours: a full stop is the only boundary read.
...
```jsonc
{
  ...
  "blocks": [{ "kind": "prose", "text": "…", "html": "…<br />…" }], // "prose" | "versicle" | "response" | "petitions" | "attribution"; `html` only when the block prints on more than one line, see below
...
- **`attribution` is the credit the page prints UNDER the prayer, and it is declared by the curation rather than recognised from its shape** (added 2026-09-06). Vatican News sets `(Pope Francis, Amoris Laetitia, 325)` under the Prayer to the Holy Family, `(Pope Francis, Patris Corde)` under the Prayer to St Joseph, and the author of each act under the two acts of spiritual communion — 22 blocks across six languages. The shape it takes differs per page: its own paragraph for the first two, the last line of the prayer's own block for the third, which is why the block boundary is something curation moves rather than something the parse finds. **A wholly-parenthesised line is not evidence of a credit** and detecting one would be wrong in both directions: the Rosary's groups print `(on Monday and Saturday)` as a rubric, and four Spanish prayers print `(tradición española)` as a provenance note inside the prayer's own run. So a prayer's curated file carries `"attribution": true` and `lift_attribution` lifts the line; a prayer that declares it and whose witnesses print none is a curation error and says so. A page that credits the prayer in some languages and not others is silence rather than an error — the German and Portuguese spiritual communion is uncredited. **Nothing is ever added**: the four devotions here with famous authors and no printed credit (St Michael's Leo XIII, the chaplet's St Faustina) keep that fact in a `flag`, because a curation may record what a page does not print but must not print it as though the page had.
...
- **Provenance is per prayer, not per work** (added 2026-08-25). A prayer collection is the one work type assembled from many unrelated pages: the English manifest lists eight `sources` and cannot say which prayer came from which, so a consumer reading `manifest.sources[0]` — which is what the site's copyright notice did — attributed all 28 to the Compendium's Appendix A. Four are not from it (the two Creeds and the Our Father come from the Catechism's own pages, the Litany of Loreto from the Holy Rosary micro-site), and the Rosary is only partly from it. So each prayer carries its own **`sources`**, in the manifest's `{url, retrieved_at}` shape.
  - **The Rosary names TWO pages, and it is the only entry that does** (2026-09-09), being the only one assembled from more than one. Its `blocks` — the title, the rubric, the concluding prayer — are the Compendium's appendix entry; its twenty mysteries and its directions, which are the bulk of what a reader sees, come from four Holy Rosary micro-site pages. Neither address answers for the other's text, so the six editions that print the mysteries cite both: the micro-site's own index **in their own language** (`index_rosary_ge.htm`, `index_rosary_sp.htm`, `index_rosary_fr.htm`, `index_rosary_it.htm`, `index_rosary_po.htm`, and the bare stem for English) first, the appendix second. Four pages of one site are one thing to cite, and the index is the site those four are pages OF. The editions with no mysteries cite the appendix alone, having no other page.
  - **`groups[].source` and `instructions.source` are gone.** They named the four sub-pages a section was parsed from, and the site printed each beside its heading — a repetition of the site the index now names, not a second address. The curation records the pair as `url` (the page the witness was fetched from, which is also what `witness` names and what `prayers.py --verify-curated` checks every word of `blocks` against) and `cite` (the address this project sends a reader to, the one field on a prayer that is not a page the scraper fetched); `prayers_project.py` projects both into `sources`, `cite` first. The work manifest's list is the union of what its prayers name, so the two can no longer disagree — which is what `check_source_coverage`'s two-way assertion was for while the collection was parsed.
