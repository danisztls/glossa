...
- **Provenance is per prayer, not per work** (added 2026-08-25). A prayer collection is the one work type assembled from many unrelated pages: the English manifest lists eight `sources` and cannot say which prayer came from which, so a consumer reading `manifest.sources[0]` — which is what the site's copyright notice did — attributed all 28 to the Compendium's Appendix A. Four are not from it (the two Creeds and the Our Father come from the Catechism's own pages, the Litany of Loreto from the Holy Rosary micro-site), and the Rosary is only partly from it. So each prayer carries its own **`sources`**, in the manifest's `{url, retrieved_at}` shape.
  - **The Rosary is cited to the site, not to the page** (2026-09-09), because it is the only entry assembled from more than one. Its `blocks` — the title, the rubric, the concluding prayer — are the Compendium's appendix entry; its twenty mysteries and its directions, which are the bulk of what a reader sees, come from four Holy Rosary micro-site pages. So the six editions that print the mysteries name that micro-site's own index **in their own language** (`index_rosary_ge.htm`, `index_rosary_sp.htm`, `index_rosary_fr.htm`, `index_rosary_it.htm`, `index_rosary_po.htm`, and the bare stem for English), and the editions with no mysteries name the appendix, having no other page. Four pages of one site are one thing to cite, and the site is where a reader is being sent.
  - **`groups[].source` and `instructions.source` are gone with it.** They named the four pages a section was parsed from, and the site printed them beside their headings — a second provenance under the first, on a page whose top-level notice named a page most of the text is not on. One address per edition is the whole answer now; what the text was CHECKED against is `witness` in `authored/prayers/`, which `prayers.py --verify-curated` reads and which still names the Compendium for all six. A curated prayer's `sources` is one entry, and the work manifest's list is the union of what its prayers name (`prayers_project.py`), so the two can no longer disagree — which is what `check_source_coverage`'s two-way assertion was for while the collection was parsed.
