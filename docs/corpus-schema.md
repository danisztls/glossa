...
A prayer collection has no numbered units in its source at all — unlike CCC paragraphs, Compendium questions, or document sections, none of which this schema invents a number for. Work IDs `prayer.{slug}.{lang}`, where `{lang}` is a full BCP-47 tag and may carry a region — `prayer.common.{de,en,en-gb,es,fr,hu,it,la,pt,ro,sl,sv}` (currently one slug: `prayer.common.{lang}`, combining the Compendium of the CCC's Appendix A, the Apostles' Creed, Nicene Creed, and Our Father re-parsed from cached CCC pages, plus the Litany of Loreto — but **not every edition has every part**, because the three source families are not published in the same languages: see the symmetry bullet below); manifest `type: "prayer"`. Array ordered by `n`:
...
`appendix.json` carries what the edition prints outside the numbered flow, in source order: Cardinal Sodano's letter of transmittal, Cardinal Martino's presentation, and the back-of-book index of references. **The letter's own paragraph numbers are kept as text**, because the appendix has no `n` to put them in and the edition prints them.
