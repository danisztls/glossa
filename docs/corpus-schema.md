...
- **Latin is a field, not an edition.** A prayer's `latin` sits beside its vernacular `blocks` on the same array entry rather than living in a `prayer.common.la` work, deliberately: the source itself only ever prints Latin as a bound companion to the vernacular text, never as independently addressable content. The other half of this argument has expired — it read "a real Latin _edition_ would need to answer `PLAN.md`'s open 'UI language vs. content language' question (nobody wants a Latin UI, but Latin would be the first _content_ language that isn't also one)", and Latin has been an interface language since 2026-08-24. What is left is the reason that was always the stronger one: the field records what the source publishes. A `prayer.common.la` work would be an edition nobody printed. See `decisions.md`'s 2026-08-15 "Latin as next source language" entry, its 2026-08-24 Latin-interface entry, and `research/prayers.md` §3.
