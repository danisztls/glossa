...
**Still outstanding at the time of writing**: the blobs already pushed to `origin/main` are still in the public history. Untracking going forward does not remove them — purging them needs `git-filter-repo` and a force-push, which is the part that actually ends the exposure. Until that runs, this entry describes the working tree and not yet the published history.
