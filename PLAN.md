...
| 19  | **A quoted text carries no date, and the corpus is not frozen**        | Every unit this site renders is dated somewhere and dated nowhere on the page. CCC 2267 was rewritten in 2018; CSDC 405, written in 2004, quotes the paragraph it replaced; _Fiducia Supplicans_ postdates all three documents `homosexualitas` names. A reader meets them in one column, in one type, with nothing to say one is older — and `/quaestiones` is only where it shows first, since `/catechismus`, `/doctrina-socialis` and `/documenta` each render the same undated text. `docs/research/topics.md`'s blocklist already refuses topics for texts the corpus LACKS, on the ground that a stale answer reads as a current one; this is that failure arriving from texts the corpus HAS. Withholding the older text is not the answer and was tried and reverted (`docs/research/topic-anchor-review.md`, 2026-09-10, by direction): choosing between two magisterial texts is a gloss with no words in it. | Half the fact is already in the corpus. Every document manifest carries `promulgated`, and so do the Compendium's (2004-04-02) and the Code's (1983-01-25); the Catechism's is `null` and its `edition` names the 1993/1997 second typical edition instead. What is in no manifest is the REVISION of a unit — `raw/<source>/captured-at.json` records when a page was FETCHED, which is a third fact again. Nobody has counted how many units have been revised since publication; CCC 2267 is the one with a name. | Not sized, and a decision before it is a change. The cheap end is reading the date the manifest already has; the expensive end is per unit with a revision history. What it must not become is a sentence of ours between a topic's heading and its quotation, which is the one thing `/quaestiones` promises not to do.                                                                                                                                                                                                                             |
