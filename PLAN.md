...
**Recommended order** (a priority argument, not a dependency one; #12, #14, #13's Compendium half and #1's scrape left the list when they shipped):

1. **The structure trees' three reader-visible defects** — moved up now that #13's Compendium half has shipped.
2. **The structure trees' remaining defects** — the leaked contents opening §1 (18 works), the drop cap read as a heading (5), and `santateresa-delbambinogesu.en`'s three lost headings, then the tier discriminator. #13's Chinese half went on 2026-09-05: it needed neither of the two things this row predicted (its Scripture links came from `book-forms-oracle.mjs --derive zht`, and no CJK webfont is coming — `direction.css` had settled that for the chrome already), and it needed one thing nothing predicted, which is a reader that can tell a paragraph it read from one it only appears to have read.
...
4. **#15's three remaining languages** — Polish, Russian and Romanian. German, French, Spanish, Italian and Hungarian shipped on 2026-08-28, Spanish included: Straubinger's manifest records `copyright.status: copyrighted` and the exposure was knowingly accepted as the self-resolving Matos Soares kind, so 1 Jan 2027 is when the term lapses rather than when the work could start. What is left is not scraping but the three decisions in the loose-ends section above, and none of them is a parser's to take. Deferred, not declined
5. **#13's Arabic Catechism and its six documents** — the normalisation layer has somewhere to grow now, and the documents are the smallest surface of the two.
6. **#2 search** — the largest unscoped item; needs a prototype before it can be planned.
7. **#1's cross-language oracle** — what the symmetry check should assert now that Latin is inside it; scoped below.
8. **#5 `related` / #6 Appendix B** — lowest urgency; each still needs a research pass in `docs/research/` style before implementation is scopeable.
