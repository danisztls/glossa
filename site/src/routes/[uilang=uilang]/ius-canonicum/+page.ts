 ...
 * in the address: `/pt/ius-canonicum` can be written out in Portuguese at build
 * time, where the bare path can only be written in English and swapped once the
 * reader's own dictionary lands.
 *
 * THE ROUTE ITSELF IS NEW (2026-09-11). `/ius-canonicum` has been in
 * `CHROME_PATHS` — so the sitemap has published `/pt/ius-canonicum` and the
