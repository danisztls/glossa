...
export interface IndexRow {
	node: StructureNode;
	depth: number;
	/** This row's identity, unique across the whole list — `rowKey` where that
	 *  is already unique, and `rowKey` plus an occurrence number where the
	 *  outline repeats itself. THE UNIQUENESS IS THE POINT: it keys the
	 *  `{#each}` that draws the index, and Svelte throws `each_key_duplicate`
	 *  on a repeat — a blank page for the whole route, not a doubled row. It
	 *  reached the Catechism's index in September 2026, from five `sub` nodes
	 *  the Portuguese parse made out of one restated running banner
	 *  (`take_mini_header`, `ccc.py`). That parse is fixed, but the corpus is
	 *  regenerated by every worktree and the index draws four works, so the
	 *  invariant is enforced here rather than assumed of the data. */
	key: string;
	/** `key` of every ancestor, outermost first. */
	ancestors: string[];
	/** Whether opening this row would reveal anything. */
	expandable: boolean;
}

/** A row's identity BEFORE disambiguation — title plus span, because a
 *  `StructureNode` carries no id and the same object is not handed back across
 *  a language switch. `IndexRow.key` is what a caller wants; this is what it
 *  is built from. */
...
export function indexRows(
	...
	const rows: IndexRow[] = [];
	// How many rows have carried each `rowKey` so far. A second one gets `#2`
	// appended and so on, which keeps the first row's key — and so the reader's
	// open/closed state for every row in a well-formed outline — exactly what
	// it was.
	const seen = new Map<string, number>();
	const uniqueKey = (node: StructureNode): string => {
	...
	const walk = (nodes: StructureNode[], depth: number, ancestors: string[]) => {
		for (const node of nodes) {
			...
			const key = uniqueKey(node);
			rows.push({ node, depth, key, ancestors, expandable: kids.length > 0 });
			if (kids.length > 0) walk(kids, depth + 1, [...ancestors, key]);
		}
	};
	walk(
