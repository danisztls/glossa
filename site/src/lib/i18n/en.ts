...
export const en: Dictionary = {
	...
	//
	// TWO KEYS, AND THE PAGE'S WHOLE BILL. Everything else on `/` is a name or
	// a sentence some other page already wrote in all thirty-seven languages:
	// each card is titled by `nav.*` and described by the tagline its own
	// landing page carries. That is the same discipline `/bibliotheca` states
	// for itself.
	//
	// **`/` IS IN `CHROME_PATHS` AND STAYS THERE** (2026-09-06, by direction),
	// which these were the one exception to until they were translated later
	// the same day. `route-manifest.ts` holds three pages out of that
	...
	// and no `hreflang` cluster is worse than one whose hidden section heading
	// falls through to English. What the rule
	...
	// The heading over the catalogue. NOT "Where to begin", which is
	...
	'document.kind.cdfDecree': 'CDF Decree',
	'document.kind.cdfNorms': 'CDF Norms',
	'document.kind.cdfNotification': 'CDF Notification',
	'document.kind.cdfCommunication': 'CDF Communication',
	...
	// in circulation and about the act, never about Freemasons — which is
	// both the defensible form and the one that actually moves somebody who
	// believes the claims. It OPENS on the act and CLOSES on the way back:
	// a note that convicts and offers no remedy is half a note, and the
	// heading is already the disclosure, so a first paragraph restating that
	// this is ours would only cost the act its place at the front.
	'quaestiones.associationes-massonicae.editorial':
		'Belonging to Freemasonry is grave sin. It is not a lapse of taste, and not a rival loyalty the Church would merely prefer you did not have: what the Congregation for the Doctrine of the Faith says is that Masonic principles “have always been considered irreconcilable with the doctrine of the Church”, and the objection is to those principles. “Irreconcilable” is the exact word and it is not a figure of speech: they are not two sets of commitments a person can hold at the same time. Nothing about a particular lodge’s charity, its respectability, or the good character of the men in it touches the question.\n\nWhat changed in 1983 is the penalty, and only the penalty. The 1917 Code excommunicated a Catholic automatically for enrolling; the Code in force since 27 November 1983 does not — and no canon can tell you that, because a canon does not print the penalties it declines to impose, which is why nothing below will say it. But that Code took effect the day after the Congregation said the Church’s judgment “remains unchanged”, that membership “remains forbidden”, and that those who enrol “are in a state of grave sin and may not receive Holy Communion”. A penalty was withdrawn. Nothing was permitted. Some bishops have since attached a penalty of their own, which can. 1315 lets them do, so the answer can be stricter where you live and is never laxer. The old canon is 2335, and the 1981 declaration listed below is where you can read that it existed: it opens by naming the canon “which forbids Catholics, under the penalty of excommunication, to enroll in Masonic or other similar associations”.\n\nThe law names no association by name, and that is deliberate rather than an oversight. It forbids joining “an association which plots against the Church”, and the 1983 declaration says Masonic bodies went unnamed because they “are contained in wider categories”. A rule written against a NAME would forbid the name and leave the same thing under another one looking permitted. So the prohibition follows what an association is and does, and a lodge, order or society that answers the description is not outside it for being called something else. Nor is that a judgement you are asked to make for yourself: the same declaration reserves it, and reserves it even from your own bishops’ conference.\n\nThis page is about an act, not about anyone’s standing before God, and it is not meant to leave a reader where it found them. The way back is the ordinary one — leave, and confess it. CCC 1385, printed below, is the whole of what stands between a person in this position and Communion.',
	// APPARATUS, NOT ARGUMENT, and required of any topic that carries a note:
	// this is the one paragraph on the site whose claims rest on nobody else's
	// authority, so it is the one that owes a reader the means to check it.
	//
	// IT SPELLS NO CITATION, and a translator writes none. `{sources}` is where
	// the page splices the units named in this topic's `editorialSources`,
	// written by `citationFor` in the reader's own language and each one a
	// link — so the siglum is `CCC` here and `CIC` in Portuguese without
	// anybody deciding that twice, and the list's own punctuation comes from
	// `Intl.ListFormat` rather than from a comma somebody typed. Keep the
	// placeholder; a translation without it publishes a note with no
	// apparatus. What is left to write is the clause around it, and the last
	// sentence is the only honest thing to say about a text the corpus does
	// not hold — which is exactly what a list of addresses cannot say.
	'quaestiones.associationes-massonicae.editorial.sources':
		'Everything above is printed or linked on this page: {sources}. The 1917 Code itself is not reproduced here.',
