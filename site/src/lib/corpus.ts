...
import { CONTENT_LANG_FALLBACK, contentLangChain, fallbackFor } from './content-fallback';
import { EMPTY_SECTION_HEADINGS, type SectionHeadings } from './section-headings';
...
/** The content-language chain, defined in `content-fallback.ts` — a leaf the
 *  build scripts can import — and re-exported here, which is where every
 *  reader of it already looks. */
export { CONTENT_LANG_FALLBACK, contentLangChain };
...
/**
 * The headings printed INSIDE the works, for the jump box — one shard, the
 * reader's own interface language, resolved at build time to the edition
 * each work will actually open in (`src/lib/section-headings.ts`).
 *
 * One request, for every heading at once, issued by `JumpBox` when the box is
 * first opened and by nothing else. The empty table for a build with no
 * shards — the fixtures, a partial sync — where the box then completes the
 * works' own names as it always did.
 *
 * IT IS NOT PRIMED AND HAS NO REGISTRY, unlike everything in
 * `ensureAllIndexes`. Those answer "does this address exist" from render, so
 * they must be resident and synchronous; this answers a question only one
 * component asks, at a moment it can await, and `suggest()` takes it as an
 * ARGUMENT — which is the rule that module states about its own language and
 * for the same reason: a function that half-reads its inputs from a global is
 * one nobody can test.
 */
export async function loadSectionHeadings(lang: string): Promise<SectionHeadings> {
	await ensureContentIndex();
	if (!USE_REAL_CORPUS) return EMPTY_SECTION_HEADINGS;
	const location = sectionHeadingsLocation(lang);
	if (!location) return EMPTY_SECTION_HEADINGS;
	return readContent<SectionHeadings>(location);
}
